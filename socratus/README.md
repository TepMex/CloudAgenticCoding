...
For the same asset paths as **GitHub Pages** on a **project site** (repo `my-repo` → `/my-repo/socratus/`):
...
On push to `master`, CI builds from `socratus/` with `GH_PAGES_PUBLIC_PATH=/<repository>/socratus/` (from the workflow) and deploys to `gh-pages` so the app is available at:

`https://<owner>.github.io/<repository>/socratus/`

Enable GitHub Pages from the `gh-pages` branch / root in repository settings if needed.

## Cursor

See `.cursor/skills/github-pages-socratus/SKILL.md` for deploy layout and path conventions.
