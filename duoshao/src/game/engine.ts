...
export class GameEngine {
    ...
    for (const target of this.targets.filter((item) => item.state !== "hit")) {
      ...
      if (now >= target.boundaryAt) {
        const timelyAttempt = [...this.attempts.values()].some(
          (attempt) => !attempt.resolved && attempt.speechStartedAt <= target.boundaryAt,
        );
        if (timelyAttempt) {
  ...
  speechStarted(id: string, at: number): void {
    if (this.phase === "running") this.attempts.set(id, { id, speechStartedAt: at, resolved: false });
  }
  ...
  recognitionResult(id: string, amount: number | null, completedAt: number): { hit: boolean } {
    ...
    const matching = amount === null ? undefined : this.targets.find(
      (target) => target.amount === amount && target.state !== "hit" &&
        (!attempt || attempt.speechStartedAt <= target.boundaryAt),
    );
    if (matching) {
      matching.state = "hit";
      matching.hitAt = completedAt;
      this.hits += 1;
      const reactionMs = attempt ? Math.max(0, attempt.speechStartedAt - matching.spawnedAt) : matching.fallDurationMs;
  ...
  position(target: FallingTarget, now: number): number {
    return Math.min(1, Math.max(0, (now - target.spawnedAt) / target.fallDurationMs));
  }
  ...
  private spawnDueTarget(now: number): void {
    ...
    const activeAmounts = new Set(this.targets.filter((target) => target.state !== "hit").map((target) => target.amount));
    let amount = settings.level === 1 && this.lastAmount === undefined ? 7 : this.difficulty.generateAmount(settings.level, this.random);
    for (let tries = 0; tries < 30 && (activeAmounts.has(amount) || amount === this.lastAmount); tries += 1) {
      amount = this.difficulty.generateAmount(settings.level, this.random);
    }
    if (activeAmounts.has(amount)) return;
